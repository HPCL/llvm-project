...
// PRT-AST-OA-NEXT: {{^ *}}// #pragma acc routine seq{{$}}
//      PRT-O-NEXT: {{^ *}}#pragma omp declare target{{$}}
// PRT-SRC-OA-NEXT: {{^ *}}// #pragma acc routine seq{{$}}
...
//  DMP-NOT: FunctionDecl
//      DMP:   ACCRoutineDeclAttr {{.*}}> Inherited Seq{{$}}
...
// This case is special because the attribute must be implied on the earlier
// function definition.  The implicit attribute must be represented in the
// in-memory AST in order for traditional OpenACC compilation to generate device
// code.  It must also be printed explicitly in the OpenMP source or the same
// problem would occur during OpenMP compilation after OpenACC source-to-source
// mode.  In either case, the result would be a linking error.
//
//      DMP: FunctionDecl [[#%#x,defOnDecl:]] {{.*}} defOnDecl 'void ()'
//  DMP-NOT: FunctionDecl
//      DMP:   ACCRoutineDeclAttr {{.*}}> Implicit Seq{{$}}
